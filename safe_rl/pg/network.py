...
#import  tensorflow.compat.v1.variables_initializer as tf3
...
def placeholder(dim=None):
    return tf2.placeholder(dtype=tf.float32, shape=combined_shape(None,dim))
...
def mlp_squashed_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    std = tf.exp(log_std)
    u = mu + tf.compat.v1.random_normal(tf.shape(mu)) * std
    pi = tf.tanh(u)
